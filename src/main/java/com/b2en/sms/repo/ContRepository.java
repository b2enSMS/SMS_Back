...
public interface ContRepository extends JpaRepository<Cont, Integer>{
	 ...
	 * 
	 * @Query(
	 * value="UPDATE cont SET cust_id = null, org_id = :#{#paramCont.org.orgId}, emp_id = :#{#paramCont.b2en.empId}, head_cont_id = :#{#paramCont.headContId}, cont_nm = :#{#paramCont.contNm}, cont_dt = :#{#paramCont.contDt}, cont_tot_amt = :#{#paramCont.contTotAmt}, cont_report_no = :#{#paramCont.contReportNo}, cont_tp_cd = :#{#paramCont.contTpCd}, install_dt = :#{#paramCont.installDt}, check_dt = :#{#paramCont.checkDt}, mtnc_start_dt = :#{#paramCont.mtncStartDt}, mtnc_end_dt = #{#paramCont.mtncEndDt} WHERE cont_id = :#{#paramCont.contId}"
	 * , nativeQuery = true) int forceUpdate(@Param("paramCont") Cont cont);
	 */
	@Transactional
	@Modifying
