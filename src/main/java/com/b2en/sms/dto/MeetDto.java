...
public class MeetDto {
	...
	@Pattern(regexp="^([12]\\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[12]\\d|3[01]))$", message="미팅일시는 yyyy-MM-dd의 형식으로, 올바른 값이 입력되어야 합니다")
	private String meetDt;
	
	@NotBlank(message="미팅내용이 빈칸입니다.")
	private String meetCnt;

	@NotBlank(message="미팅시작시간이 빈칸입니다.")
	@Pattern(regexp="^\\d{2}:\\d{2}:\\d{2}$", message="미팅시작시간은 hh:mm:ss의 형식으로 값이 입력되어야 합니다")
